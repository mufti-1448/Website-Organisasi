<?php

namespace App\Http\Controllers;

use App\Models\PengaturanWeb;
use App\Models\Carousel;
use App\Models\ProfilOrganisasi;
use App\Models\TentangKami;
use App\Http\Requests\StorePengaturanWebRequest;
use App\Http\Requests\UpdatePengaturanWebRequest;
use App\Http\Requests\StoreCarouselRequest;
use App\Http\Requests\UpdateCarouselRequest;
use App\Http\Requests\StoreTentangKamiRequest;
use App\Http\Requests\UpdateTentangKamiRequest;
use App\Http\Requests\StoreProfilOrganisasiRequest;
use App\Http\Requests\UpdateProfilOrganisasiRequest;
use Illuminate\Support\Facades\Storage;
use Illuminate\Support\Str;
use Illuminate\Http\Request;

class PengaturanWebController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index()
    {
        $pengaturan = PengaturanWeb::first(); // Asumsi hanya satu record
        $carousels = Carousel::all();
        $profil = ProfilOrganisasi::first();
        $tentang = TentangKami::first();
        return view('pengaturan_web.index', compact('pengaturan', 'carousels', 'profil', 'tentang'));
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create()
    {
        return view('pengaturan_web.create');
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(StorePengaturanWebRequest $request)
    {
        $validated = $request->validated();

        $logoPath = $request->file('logo')->store('uploads/logo', 'public');
        $faviconPath = $request->file('favicon')->store('uploads/favicon', 'public');

        PengaturanWeb::create(array_merge($validated, [
            'logo' => $logoPath,
            'favicon' => $faviconPath,
        ]));

        return redirect()->route('pengaturan-web.index')->with('success', 'Pengaturan Web berhasil ditambahkan.');
    }

    /**
     * Display the specified resource.
     */
    public function show(string $id)
    {
        $pengaturan = PengaturanWeb::findOrFail($id);
        return view('pengaturan_web.show', compact('pengaturan'));
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(string $id)
    {
        $pengaturan = PengaturanWeb::findOrFail($id);
        return view('pengaturan_web.edit', compact('pengaturan'));
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(UpdatePengaturanWebRequest $request, string $id)
    {
        $pengaturan = PengaturanWeb::findOrFail($id);
        $validated = $request->validated();

        if ($request->hasFile('logo')) {
            // Hapus logo lama
            if ($pengaturan->logo && Storage::disk('public')->exists($pengaturan->logo)) {
                Storage::disk('public')->delete($pengaturan->logo);
            }
            $validated['logo'] = $request->file('logo')->store('uploads/logo', 'public');
        }

        if ($request->hasFile('favicon')) {
            // Hapus favicon lama
            if ($pengaturan->favicon && Storage::disk('public')->exists($pengaturan->favicon)) {
                Storage::disk('public')->delete($pengaturan->favicon);
            }
            $validated['favicon'] = $request->file('favicon')->store('uploads/favicon', 'public');
        }

        $pengaturan->update($validated);

        return redirect()->route('pengaturan-web.index')->with('success', 'Pengaturan Web berhasil diperbarui.');
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(string $id)
    {
        $pengaturan = PengaturanWeb::findOrFail($id);

        // Hapus logo dari storage
        if ($pengaturan->logo && Storage::disk('public')->exists($pengaturan->logo)) {
            Storage::disk('public')->delete($pengaturan->logo);
        }

        // Hapus favicon dari storage
        if ($pengaturan->favicon && Storage::disk('public')->exists($pengaturan->favicon)) {
            Storage::disk('public')->delete($pengaturan->favicon);
        }

        $pengaturan->delete();

        return redirect()->route('pengaturan-web.index')->with('success', 'Pengaturan Web berhasil dihapus.');
    }

    // Carousel methods
    public function storeCarousel(StoreCarouselRequest $request)
    {
        $validated = $request->validated();
        $validated['gambar'] = $request->file('gambar')->store('uploads/carousel', 'public');
        Carousel::create($validated);
        return redirect()->route('pengaturan-web.index')->with('success', 'Carousel berhasil ditambahkan.');
    }

    public function updateCarousel(UpdateCarouselRequest $request, $id)
    {
        $carousel = Carousel::findOrFail($id);
        $validated = $request->validated();

        if ($request->hasFile('gambar')) {
            if ($carousel->gambar && Storage::disk('public')->exists($carousel->gambar)) {
                Storage::disk('public')->delete($carousel->gambar);
            }
            $validated['gambar'] = $request->file('gambar')->store('uploads/carousel', 'public');
        }

        $carousel->update($validated);
        return redirect()->route('pengaturan-web.index')->with('success', 'Carousel berhasil diperbarui.');
    }

    public function destroyCarousel($id)
    {
        $carousel = Carousel::findOrFail($id);
        if ($carousel->gambar && Storage::disk('public')->exists($carousel->gambar)) {
            Storage::disk('public')->delete($carousel->gambar);
        }
        $carousel->delete();
        return redirect()->route('pengaturan-web.index')->with('success', 'Carousel berhasil dihapus.');
    }

    // Profil Organisasi methods
    public function updateProfil(UpdateProfilOrganisasiRequest $request)
    {
        $profil = ProfilOrganisasi::first();
        if (!$profil) {
            $profil = new ProfilOrganisasi();
        }

        $validated = $request->validated();

        $profil->fill($validated)->save();
        return redirect()->route('pengaturan-web.index')->with('success', 'Profil Organisasi berhasil diperbarui.');
    }

    // Tentang Kami methods
    public function updateTentang(StoreTentangKamiRequest $request)
    {
        $tentang = TentangKami::first();
        if (!$tentang) {
            $tentang = new TentangKami();
        }

        $validated = $request->validated();
        $tentang->fill($validated)->save();
        return redirect()->route('pengaturan-web.index')->with('success', 'Tentang Kami berhasil diperbarui.');
    }

    // Public frontend methods
    public function home()
    {
        $pengaturan = PengaturanWeb::first();
        $carousels = Carousel::all();
        $profil = ProfilOrganisasi::first();
        return view('frontend.home', compact('pengaturan', 'carousels', 'profil'));
    }

    public function about()
    {
        $pengaturan = PengaturanWeb::first();
        $profil = ProfilOrganisasi::first();
        $tentang = TentangKami::first();
        return view('frontend.about', compact('pengaturan', 'profil', 'tentang'));
    }
}
