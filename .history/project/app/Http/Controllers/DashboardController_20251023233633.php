...
class DashboardController extends Controller
{
    ...
    public function getMonthlyData()
    {
        ...
        $rapatMonths = Rapat::selectRaw('YEAR(tanggal) as year, MONTH(tanggal) as month')
            ->groupBy('year', 'month')
            ->get();

        // Get all unique months with program data
        $programMonths = ProgramKerja::selectRaw('YEAR(target_date) as year, MONTH(target_date) as month')
            ->groupBy('year', 'month')
            ->get();
