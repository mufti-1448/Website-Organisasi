...
class TentangKamiController extends Controller
{
    ...
    public function store(StoreTentangKamiRequest $request)
    {
        ...
        $validated['id'] = $request->input('id', 'TKM' . str_pad(TentangKami::count() + 1, 3, '0', STR_PAD_LEFT));

        $fotoPath = $request->file('foto')->store('uploads/tentang_kami', 'public');

        TentangKami::create(array_merge($validated, ['foto' => $fotoPath]));
     ...
     */
    public function update(UpdateTentangKamiRequest $request, string $id)
    {
        $tentangKami = TentangKami::findOrFail($id);
        $validated = $request->validated();
        ...
        if ($request->hasFile('foto')) {
            ...
            $validated['foto'] = $request->file('foto')->store('uploads/tentang_kami', 'public');
        }
