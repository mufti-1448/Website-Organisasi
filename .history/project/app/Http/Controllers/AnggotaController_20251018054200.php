...
class AnggotaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($file) {
            ...
            $namaFile = time() . '_' . $request->id . '_' . $namaOrang . '.jpg';
    ...
    public function update(Request $request, string $id)
    {
        ...
        if ($file) {
            ...
            if ($anggota->foto && file_exists(public_path('uploads/anggota/' . $anggota->foto))) {
                unlink(public_path('uploads/anggota/' . $anggota->foto));
            }
            // Membuat nama file baru dengan format: timestamp_id_nama.jpg
            $namaOrang = str_replace(' ', '_', strtolower($request->nama));
            $ekstensi  = $file->getClientOriginalExtension();
            $namaFile = time() . '_' . $id . '_' . $namaOrang . '.' . $ekstensi;
    ...
    public function destroy(string $id)
    {
        ...
        if ($anggota && $anggota->foto) {
            $filePath = public_path('uploads/anggota/' . $anggota->foto);
