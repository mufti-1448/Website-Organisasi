...
class RapatController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        // Cek apakah request mengandung notulen_id
        if ($request->notulen_id) {
            // Lepaskan notulen lama (jika ada)
            Notulen::where('rapat_id', $rapat->id)
                ->where('id', '!=', $request->notulen_id)
                ->update(['rapat_id' => $rapat->id]); // tetap isi rapat_id yang sama (tidak null)

            // Hubungkan notulen baru
            $notulen = Notulen::find($request->notulen_id);
            if ($notulen) {
                $notulen->rapat_id = $rapat->id;
                $notulen->save();
