...
class RapatController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($lastRapat) {
            // Ambil angka terakhir dari ID (contoh: RPT005 → 5)
            $num = (int)substr($lastRapat->id, 3) + 1;
            $nextCode = 'RPT' . str_pad($num, 3, '0', STR_PAD_LEFT);
        }
        ...
        DB::table('rapat')->insert([
            ...
            'status' => $request->status, // ganti dari 'keterangan' ke 'status'
            'jumlah_peserta' => $request->jumlah_peserta ?? null, // tambahkan kolom baru
            'created_at' => now(),
            'updated_at' => now(),
        ]);


        return redirect()->route('rapat.index')->with('success', 'Rapat berhasil ditambahkan dengan ID ' . $nextCode);
    }
