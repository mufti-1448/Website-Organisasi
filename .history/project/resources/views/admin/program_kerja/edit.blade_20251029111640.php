...
@section('content')
    <div class="container mt-4">
        ...
        <form action="{{ route('program_kerja.update', $program->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="notulen_id" class="form-select">
                            ...
                            @foreach ($notulenList as $not)
                                <option value="{{ $not->id }}"
                                    {{ optional($program->notulen->first())->id == $not->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="dokumentasi_id" class="form-select">
                            ...
                            @foreach ($dokumentasiList as $dok)
                                <option value="{{ $dok->id }}"
                                    {{ optional($program->dokumentasi->first())->id == $dok->id ? 'selected' : '' }}>
                                    {{ $dok->judul ?? 'Dokumentasi' }} 
                                    ({{ \Carbon\Carbon::parse($dok->tanggal)->format('d-m-Y') }})
                                </option>
                            @endforeach
                        </select>
                    </div>
            ...
            <button type="submit" class="btn btn- mt-3"><i class="bi bi-save"></i> Update</button>
