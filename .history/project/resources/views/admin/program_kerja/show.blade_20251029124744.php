            ...
            <form>
                ...
                <div class="tab-content mt-3">
                    ...
                    <div class="tab-pane fade" id="notulen">
                        @if ($program->notulen->count() > 0)
                            <div class="mb-3">
                                <label class="form-label">Isi Notulen</label>
                                <textarea class="form-control" rows="5" readonly>{{ $program->notulen->first()->isi }}</textarea>
                            </div>
                            <div class="row">
                                <div class="col-md-4">
                                    <label class="form-label">Tanggal</label>
                                    <input type="date" class="form-control"
                                        value="{{ $program->notulen->first()->tanggal }}" readonly>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">Waktu</label>
                                    <input type="time" class="form-control"
                                        value="{{ $program->notulen->first()->waktu }}" readonly>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">Penulis</label>
                                    <input type="text" class="form-control"
                                        value="{{ $program->notulen->first()->penulis->nama ?? '-' }}" readonly>
                                </div>
                            </div>
                        @else
                    ...
                    <div class="tab-pane fade" id="dokumentasi">
                        @if ($program->dokumentasi->count() > 0)
                            <div class="mb-3">
                                <label class="form-label">Judul Dokumentasi</label>
                                <input type="text" class="form-control"
                                    value="{{ $program->dokumentasi->first()->judul }}" readonly>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Deskripsi</label>
                                <textarea class="form-control" rows="3" readonly>{{ $program->dokumentasi->first()->deskripsi ?? '-' }}</textarea>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Tanggal</label>
                                <input type="date" class="form-control"
                                    value="{{ $program->dokumentasi->first()->tanggal }}" readonly>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Kategori</label>
                                <input type="text" class="form-control"
                                    value="{{ $program->dokumentasi->first()->kategori ?? '-' }}" readonly>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Foto</label>
                                @if ($program->dokumentasi->first()->foto)
                                    <br>
                                    <img src="{{ asset('storage/' . $program->dokumentasi->first()->foto) }}"
                ...
                <div class="tab-pane fade" id="evaluasi">
                    @if ($program->evaluasi->count() > 0)
                        <div class="mb-3">
                            <label class="form-label">Judul Evaluasi</label>
                            <input type="text" class="form-control" value="{{ $program->evaluasi->first()->judul }}" readonly>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Isi Evaluasi</label>
                            <textarea class="form-control" rows="5" readonly>{{ $program->evaluasi->first()->isi }}</textarea>
                        </div>
                        <div class="row">
                            <div class="col-md-4">
                                <label class="form-label">Tanggal</label>
                                <input type="date" class="form-control" value="{{ $program->evaluasi->first()->tanggal }}"
                                    readonly>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Penulis</label>
                                <input type="text" class="form-control"
                                    value="{{ $program->evaluasi->first()->penulis->nama ?? '-' }}" readonly>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">File</label>
                                @if ($program->evaluasi->first()->file)
                                    <br>
                                    <a href="{{ asset('storage/' . $program->evaluasi->first()->file) }}" target="_blank"
            </form>

            <div class="gap-3">
                <a href="{{ route('program_kerja.index') }}" class="btn btn-secondary mt-3"><i
                        class="bi bi-arrow-left"></i>
                    Kembali</a>
                <a href="{{ route('program_kerja.edit', $program->id) }}" class="btn btn-warning mt-3"><i
