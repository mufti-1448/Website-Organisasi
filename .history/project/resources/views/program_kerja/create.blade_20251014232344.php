                ...
                <div class="mb-3">
                    ...
                    <select name="notulen_id" class="form-select">
                        ...
                        @foreach($notulenList as $not)
                            <option value="{{ $not->id }}">{{ $not->judul ?? 'Tanpa Judul' }} - {{ \Carbon\Carbon::parse($not->tanggal)->format('d-m-Y') }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="dokumentasi_ids[]" class="form-select" multiple>
                        @foreach($dokumentasiList as $doc)
                            <option value="{{ $doc->id }}">{{ $doc->judul ?? 'Dokumentasi' }} - {{ \Carbon\Carbon::parse($doc->tanggal)->format('d-m-Y') }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="evaluasi_id" class="form-select">
                        ...
                        @foreach($evaluasiList as $eval)
                            <option value="{{ $eval->id }}">{{ $eval->judul ?? 'Evaluasi' }} - {{ \Carbon\Carbon::parse($eval->tanggal)->format('d-m-Y') }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
