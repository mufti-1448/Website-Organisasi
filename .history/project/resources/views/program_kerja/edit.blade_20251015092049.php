...
<div class="container mt-4">
    ...
    <option value="{{ $not->id }}"
        {{ optional($program->notulenUtama)->id == $not->id ? 'selected' : '' }}>
        {{ $not->judul ?? 'Tanpa Judul' }} - {{ \Carbon\Carbon::parse($not->tanggal)->format('d-m-Y') }}
    </option>
@endforeach
                ...
                <div class="mb-3">
                    ...
                    <select name="dokumentasi_ids[]" class="form-select" multiple>
                        @foreach($dokumentasiList as $doc)
                            <option value="{{ $doc->id }}"
                                @if($program->dokumentasi->pluck('id')->contains($doc->id)) selected @endif>
                                {{ $doc->judul ?? 'Dokumentasi' }} - {{ \Carbon\Carbon::parse($doc->tanggal)->format('d-m-Y') }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="evaluasi_id" class="form-select">
                        ...
                        @foreach($evaluasiList as $eval)
                            <option value="{{ $eval->id }}"
                                {{ optional($program->evaluasi->first())->id == $eval->id ? 'selected' : '' }}>
                                {{ $eval->judul ?? 'Evaluasi' }} - {{ \Carbon\Carbon::parse($eval->tanggal)->format('d-m-Y') }}
                            </option>
                        @endforeach
                    </select>
                </div>
        ...
        <a href="{{ route('program_kerja.index') }}" class="btn btn-secondary mt-3"><i class="bi bi-arrow-left"></i> Kembali</a>
    </form>
</div>
@endsection
